...
const UNIVERSITY_REPUTATION: Record<string, { score: number; tier: string; strengths: string[] }> =
  {
    MSU: {
      ...
      tier: 'Tier 1 Public',
      strengths: [
        'Strong Midwest employer network',
    ...
    'Emory ECE': {
      ...
      strengths: [
        ...
        'Executive education reputation',
    ...
    'Emory GBS': {
      ...
      tier: 'Top 25 Private',
      strengths: [
        'Goizueta Business School prestige',
...
const MARKET_DEMAND: Record<
  string,
  { score: number; trend: 'rising' | 'stable' | 'declining'; notes: string }
> = {
  ...
  'Digital Transformation': {
    score: 8.5,
    trend: 'rising',
    notes: 'Enterprise priority post-pandemic',
  },
  ...
  'Six Sigma': {
    score: 6.5,
    trend: 'declining',
    notes: 'Mature methodology, less differentiation',
  },
...
function categorizeProgram(programName: string): string {
  ...
  if (name.includes('finance') || name.includes('accounting') || name.includes('financial'))
    return 'Finance';
...
function analyzePrograms(): RecommendedProgram[] {
  ...
  return validPrograms
    .map(program => {
      ...
      const reputation = UNIVERSITY_REPUTATION[program.school] || {
        score: 5,
        tier: 'Unknown',
        strengths: [],
      };
      ...
      const enrollScore = ((program.conversions?.leadToEnrollmentRate || 0) / maxEnrollRate) * 100;

      // Metrics score: weighted conversion efficiency
      const metricsScore = volumeScore * 0.3 + enrollScore * 0.7;
      ...
      const demandScore =
        demand.score * 10 +
        (demand.trend === 'rising' ? 10 : demand.trend === 'declining' ? -10 : 0);
      ...
      const overallScore =
        ...
        reputationScore * 0.2 + // Brand matters for conversions
        demandScore * 0.25 + // Market timing
        competitionScore * 0.1 + // Competitive landscape
        geoScore * 0.1; // Cost efficiency
      ...
      if (metricsScore >= 60)
        reasoning.push(
          `Strong conversion metrics (${(program.conversions?.leadToEnrollmentRate || 0).toFixed(1)}% enrollment rate)`
        );
      ...
      if (demand.trend === 'rising')
        reasoning.push(`${category} skills in high demand (trending up)`);
      else if (demand.trend === 'declining')
        reasoning.push(`${category} market showing saturation`);
      ...
      if (geo.cpmIndex < 0.8)
        reasoning.push(`Cost-efficient geo-targeting (${geo.markets.slice(0, 2).join(', ')})`);

      if (program.leads >= 500)
        reasoning.push(`Proven lead volume (${program.leads.toLocaleString()} leads)`);
    ...
    <Box sx={{ p: 4 }}>
      ...
      <Box sx={{ mb: 6 }}>
        ...
        >
          <p className="text-slate-700 mb-4">
            Based on comprehensive analysis of <strong>{analyzedPrograms.length} programs</strong>{' '}
            with 50+ leads, evaluating performance metrics, university reputation, market demand,
            competitive landscape, and geo-targeting efficiency, we recommend the following
            investment strategy:
          </p>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              ...
              >
                <span
                  ...
                  style={{ color: VERDICT_COLORS['strong-buy'].text }}
                >
                  {strongBuys.length}
                </span>
              </Box>
          ...
          {[
            {
              ...
              weight: '35%',
              icon: TrendingUp,
              desc: 'Leads, conversion rates',
            },
            {
              label: 'Market Demand',
              weight: '25%',
              icon: Users,
              desc: 'Skill trends, job market',
            },
            {
              label: 'University Reputation',
              weight: '20%',
              icon: Star,
              desc: 'Brand value, rankings',
            },
            ...
            return (
              ...
              >
                <Box sx={{ p: 3 }}>
                  ...
                  {/* Score Breakdown */}
                  <Box
                    sx={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 2, mb: 3 }}
                  >
                    {[
                  ...
                  <Box sx={{ display: 'flex', gap: 4, mb: 3, flexWrap: 'wrap' }}>
                    <Box>
                      ...
                      <p className="text-lg font-semibold text-slate-900">
                        {item.program.leads.toLocaleString()}
                      </p>
                    </Box>
                    <Box>
                      <p className="text-xs text-slate-500">Contact to Enroll</p>
                      <p className="text-lg font-semibold text-slate-900">
                        {item.program.conversions?.leadToEnrollmentRate?.toFixed(1)}%
                      </p>
                    </Box>
                    <Box>
                      <p className="text-xs text-slate-500">Lead to Enrollment</p>
                      <p className="text-lg font-semibold text-slate-900">
                        {item.program.conversions?.leadToEnrollmentRate?.toFixed(1)}%
                      </p>
                    </Box>
                  ...
                  {geo && (
                    <Box sx={{ mt: 2, display: 'flex', alignItems: 'center', gap: 1 }}>
                      ...
                      <span className="text-xs text-slate-500">
                        ...
                        {geo.cpmIndex < 1 && (
                          <span className="text-green-600 ml-2">(Lower CPM region)</span>
                        )}
                      </span>
                    </Box>
                  )}
      ...
      >
        ...
        <ul className="text-sm text-amber-800 space-y-2">
          <li>
            <strong>AI & Data Programs:</strong> Highest market demand. Prioritize Emory GBS and SMU
            offerings for premium positioning.
          </li>
          <li>
            <strong>Healthcare Programs:</strong> Strong demand with lower competition. USF and MSU
            have cost-efficient geo-targeting.
          </li>
          <li>
            <strong>Certificates vs Degrees:</strong> Certificates show faster conversion cycles.
            Consider for quick-win campaigns.
          </li>
          <li>
            <strong>Geo Strategy:</strong> ECSU and MSU offer 25-40% lower CPMs. Test broader reach
            with these programs.
          </li>
          <li>
            <strong>Avoid Over-Investment:</strong> General business and saturated project
            management programs show diminishing returns.
          </li>
        </ul>
      </Box>
    </Box>
